...
const builder = new addonBuilder({
  ...
  types: ["series"],
  catalogs: [
    { type: "series", id: "latanime-top", name: "Latanime" }
  ]
});
...
builder.defineCatalogHandler(async ({ type, id }) => {
  if (type === "series" && id === "latanime-top") {
    ...
    $(".animes .col-6").each((i, el) => {
      ...
      if (title && href && img) {
        metas.push({ id: href.split("/").pop(), type: "series", name: title, poster: img });
      }
    });
    return { metas };
  }
...
builder.defineMetaHandler(async ({ type, id }) => {
  ...
  $(".episodes-list .col-6").each((i, el) => {
    ...
    if (episodeTitle && href) {
      ...
      videos.push({ id: episodeId, title: episodeTitle, season: 1, episode: episodeNumber, released: new Date() });
    }
  });
...
builder.defineStreamHandler(async ({ type, id }) => {
  ...
  const streams = [];
  $(".cap_repro .play-video").each((i, el) => {
    ...
    try {
      ...
      if (videoUrl) streams.push({ url: videoUrl, title: $(el).text().trim() });
    } catch (e) {
...
module.exports = builder.getInterface(); // <- export AddonInterface directly
