...
const { fetchWithScrapingBee } = require('./scraping');
...
async function extractVideoUrl(context, url, referer = null) {
    ...
    if (cached) {
        console.log(`[Scraper] Cache hit for final URL: ${url}`);
        return cached;
    }
    ...
    try {
        ...
        page.on('request', request => {
            ...
            if (isValidStreamUrl(reqUrl) && !videoUrl) {
                console.log(`[Scraper] 🎯 Potential video URL detected via network: ${reqUrl.substring(0, 100)}...`);
                videoUrl = reqUrl;
            }
        });
        ...
        if (!videoUrl) {
            console.log(`[Scraper] No video found yet for ${url}, attempting to trigger play...`);
        ...
        if (!videoUrl) {
            ...
            if (extractor) {
                videoUrl = await extractor(page).catch(e => {
                    console.error(`[Scraper] Extractor error for ${url}: ${e.message}`);
                    return null;
                });
            } else {
        ...
        if (videoUrl && isValidStreamUrl(videoUrl)) {
            ...
            console.log(`[Scraper] ⚠️ Invalid stream URL detected or rejected for ${url}: ${videoUrl}`);
        } else {
            console.log(`[Scraper] ❌ No video URL found for ${url}`);
        }
        return null;
    } finally {
        if (page) {
...
async function scrapeStreams(url) {
    let browser;
    let context;
    try {
        ...
        console.log(`[Scraper] Scraping latanime page via ScrapingBee: ${url}`);
        ...
        console.log(`[Scraper] Found ${providers.length} potential providers.`);
        ...
        const processProvider = async (provider) => {
            ...
            try {
                ...
                if (finalUrl) {
                    console.log(`[Scraper] Found final embed for ${provider.title}`);
                    const videoUrl = await extractVideoUrl(context, finalUrl, provider.url);
                    if (videoUrl) {
                        console.log(`[Scraper] ✅ Extracted: ${provider.title}`);
                ...
                console.error(`[Scraper] Error processing ${provider.title}: ${e.message}`);
            } finally {
        ...
        console.log(`[Scraper] Total streams found: ${allStreams.length}`);
        return allStreams;

    } catch (error) {
        console.error(`[Scraper] Scraping error on ${url}: ${error.message}`);
        return [];
    } finally {
        if (context) {
