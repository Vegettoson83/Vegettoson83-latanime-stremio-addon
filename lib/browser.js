...
const PLACEHOLDER_KEYWORDS = [
    'placeholder', 'blank', 'dummy', 'advertisement', 'preroll'
];

// Stricter regex for "ad" to avoid false positives in valid URLs
const AD_KEYWORD_REGEX = new RegExp(`[/_-]ad([/_-]|$)`);
...
function isValidStreamUrl(url) {
    ...
    // Check for general placeholder keywords
    const hasPlaceholder = PLACEHOLDER_KEYWORDS.some(keyword => lowerUrl.includes(keyword));
    if (hasPlaceholder) {
        console.log(`[Validator] URL discarded for containing placeholder keyword: ${url}`);
        return false;
    }

    // Check for the specific, bounded "ad" keyword
    if (AD_KEYWORD_REGEX.test(lowerUrl)) {
        console.log(`[Validator] URL discarded for containing 'ad' keyword: ${url}`);
        return false;
    }

    // Check for valid extension before query string
    const mainPart = lowerUrl.split('?')[0];
    const hasValidExtension = mainPart.endsWith('.m3u8') || mainPart.endsWith('.mp4');

    if (!hasValidExtension) {
        console.log(`[Validator] URL discarded for invalid extension: ${url}`);
        return false;
    }
...
const PROVIDERS = {
    ...
    'dsvplay.com': async (page) => {
        return page.evaluate(() => {
             const scripts = Array.from(document.querySelectorAll('script'));
             for (const script of scripts) {
                const match = script.textContent.match(/sources:\s*\[{\s*file:\s*'"([^"']+)"'/);
    ...
    'mxdrop.com': async (page) => {
        return page.evaluate(() => {
            const scripts = Array.from(document.querySelectorAll('script'));
            for (const script of scripts) {
               const match = script.textContent.match(/(\w+\|video\|[^']+')/);
               if (match) {
                   const parts = match[1].split('|');
                   // Basic de-obfuscation for common packing format
                   if (parts.length > 3) {
                       return `https://s${parts[3]}.${parts[0]}.xyz/${parts[4]}/video.m3u8`;
                   }
               }
            }
            return null;
...
async function extractVideoUrl(context, url, referer = null) {
    ...
    try {
        ...
        if (isValidStreamUrl(videoUrl)) {
            ...
            if (videoUrl) { // Log if a URL was found but deemed invalid
                console.log(`[Browser] Discarded invalid or placeholder URL: ${videoUrl}`);
            }
            return null;
        }
    } finally {
