    ...
    builder.defineStreamHandler(async ({type, id}) => {
        ...
        try {
            ...
            const streamPromises = intermediateProviders.map(async (provider) => {
                try {
                    const playerHtml = await fetchWithScrapingBee(provider.url, false); // JS rendering not needed for regex

                    const match = playerHtml.match(/var redir = atob\("([^"]+)"\);/);
                    if (match && match[1]) {
                        const finalUrl = Buffer.from(match[1], 'base64').toString('utf8');
                        console.log(`Extracted final URL for ${provider.title}: ${finalUrl}`);
                        return {
                            url: finalUrl,
                    ...
                    console.warn(`Could not find 'redir' variable for ${provider.title}`);
                    return null;
                } catch (error) {
