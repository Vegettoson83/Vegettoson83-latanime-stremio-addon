...
const { getBrowser, extractVideoUrl } = require("./browserScraper");
...
function defineHandlers(builder) {
    ...
    builder.defineStreamHandler(async ({ type, id }) => {
        ...
        try {
            ...
            const basePlayerUrl = baseKey ? atob(baseKey) : null;
            ...
            $('a.play-video').each((i, el) => {
                ...
                if (encodedPart) {
                    ...
                    try {
                        ...
                        finalUrl = decoded.startsWith('http') ? decoded : (basePlayerUrl ? basePlayerUrl + encodedPart : null);
                    } catch (e) {
                        finalUrl = basePlayerUrl ? basePlayerUrl + encodedPart : null;
                    }
                    if (finalUrl) {
                        providerUrls.push({ name: providerName, url: finalUrl });
                    }
                }
            });
            ...
            console.log(`[Addon] Found ${providerUrls.length} stream providers and ${downloadLinks.length} download links. Processing sequentially...`);
            ...
            for (const provider of providerUrls) {
                try {
                    console.log(`[Addon] Scraping provider '${provider.name}': ${provider.url.substring(0, 60)}...`);
                    const videoUrl = await extractVideoUrl(browser, provider.url, targetUrl);
                    if (videoUrl) {
                    ...
                    console.error(`[Addon] ❌ Failed processing provider ${provider.name} (${provider.url}): ${e.message}`);
                }
            }

            const allStreams = [...resolvedStreams, ...downloadLinks];

            console.log(`[Addon] Successfully resolved ${resolvedStreams.length} streams. Total sources: ${allStreams.length}`);
            return { streams: allStreams };

        } catch (error) {
            console.error(`[Addon] Major error in stream handler for ${targetUrl}:`, error.message);
            if (error.response) {
                console.error('[Addon] Error Response:', error.response.data);
            }
            return Promise.resolve({ streams: [] });
        }
    });
