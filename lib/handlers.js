...
const PROVIDERS = {
    ...
    'voe.sx': async (page) => {
        ...
        return page.evaluate(() => {
            const scripts = Array.from(document.querySelectorAll('script'));
            for (const script of scripts) {
                const match = script.textContent.match(/'hls':\s*'([^']+)'|'mp4':\s*'([^']+)'/);
    ...
    'filemoon.sx': async (page) => {
        await page.waitForSelector('video', { timeout: 10000 }).catch(() => {});
        return page.evaluate(() => {
            const match = document.body.innerHTML.match(/file:\s*"([^"]+m3u8[^"]*)"/);
        ...
        try {
            ...
            const processProvider = async (provider) => {
                ...
                try {
                    ...
                    await context.route('**/*', r => (['image', 'stylesheet', 'font'].includes(r.request().resourceType()) ? r.abort() : r.continue()));

                    const page = await context.newPage();
                    await page.goto(provider.url, { waitUntil: 'domcontentloaded', timeout: 15000 }).catch(() => {});
                    const finalUrl = await page.evaluate(() => document.body.innerHTML.match(/var redir = atob\("([^"]+)"\);/) ? atob(RegExp.$1) : null).catch(() => null);
