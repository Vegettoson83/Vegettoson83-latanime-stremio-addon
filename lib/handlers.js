...
function defineHandlers(builder) {
    ...
    async function getLatanimeEpisodeUrl(imdbId, season, episode) {
        try {
            // 1. Get series name from IMDb ID using Cinemeta
            ...
            // 2. Search for the series on Latanime and select the best match for the season
                ...
                if (searchResults.length === 1) {
                    ...
                    searchResults.each((i, el) => {
                        ...
                        if (season > 1 && (title.match(seasonRegex) || href.match(seasonRegex))) {
                            bestMatch = href;
                            return false; // Found a good season match, break
                        }
                        if (season === 1 && title.match(exactMatchRegex)) {
                            bestMatch = href; // Found an exact match for S1
                        }
                    });
                    seriesUrl = bestMatch || searchResults.first().attr('href'); // Fallback to first result
                }
            }
            ...
            // 3. Scrape the series page to find the correct episode
            const seriesPageHtml = await fetchWithScrapingBee(seriesUrl);
            $ = cheerio.load(seriesPageHtml);

            let episodeUrl = null;
            // Reverse the list of episodes, as sites often list them newest-first
...
In your Render dashboard, set these:
```
PORT=10000
