...
function defineHandlers(builder) {
    ...
    builder.defineStreamHandler(async ({ type, id }) => {
        ...
        try {
            ...
            const searchHtml = await fetchWithScrapingBee(searchUrl, false);
            ...
            if (searchResults.length > 0) {
                const seasonPattern = new RegExp(`(temporada|season)[ -]*${season}`, 'i');
                // First, look for a result that explicitly matches the season
                ...
                // If no season-specific match, look for a generic entry (often for season 1)
                if (!seriesPageUrl) {
                    for (const result of searchResults) {
                        if (!/temporada|season/i.test(result.title) && !/temporada|season/i.test(result.href)) {
                            seriesPageUrl = result.href;
                            break;
                        }
                    }
            ...
            const seriesHtml = await fetchWithScrapingBee(seriesPageUrl, false);
