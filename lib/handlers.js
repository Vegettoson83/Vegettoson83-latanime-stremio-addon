...
function defineHandlers(builder) {
    ...
    async function getLatanimeEpisodeUrl(imdbId, season, episode) {
        try {
            console.log(`[Addon] [STATE-META] Resolving IMDb ID: ${imdbId} (S${season}E${episode})`);
            ...
            console.log(`[Addon] [STATE-META] Resolved ${imdbId} to "${seriesName}"`);
            ...
            if (!seriesUrl) {
                console.warn(`[Addon] [STATE-SEARCH] No series URL found for "${seriesName}" Season ${season}`);
                return null;
            }
            console.log(`[Addon] [STATE-SEARCH] Found series URL for S${season}: ${seriesUrl}`);
