...
function defineHandlers(builder) {
    builder.defineCatalogHandler(async ({type, id, extra}) => {
        ...
        try {
            ...
            if (id === 'latanime-new' && !extra?.search) {
                selector = $('h2:contains("Series recientes")').next('ul').find('li.col a');
            } else {
                selector = $('h1:contains("Animes")').next('ul').find('li.col a');
            }
            ...
            if (metas.length === 0) {
                console.log(`No items found for catalog ${id}. The CSS selectors may be outdated.`);
            } else {
                console.log(`Found ${metas.length} items in catalog ${id}`);
            }
                    ...
                    if (href && href.includes('/ver/')) {
                        ...
                        if (episodeId && !episodeMap.has(episodeId)) {
                            ...
                            if (epMatch) {
                                ...
                                // Fallback if no number is found, use index
                                const existing = Array.from(episodeMap.values());
                                episode = existing.length + 1;
                            }
                            ...
                            // Construct a more reliable ID based on the series ID and episode number
                            const finalEpisodeId = `${animeId}-episodio-${episode}`;

                            episodeMap.set(finalEpisodeId, {
                                id: `latanime-${finalEpisodeId}`,
            ...
            if (searchResults.length > 0) {
                if (searchResults.length === 1) {
                    ...
                    searchResults.each((i, el) => {
                        ...
                        if (season === 1 && title.match(exactMatchRegex)) {
                            bestMatch = href;
                        }
                    });
                    seriesUrl = bestMatch || searchResults.first().attr('href');
                }
            }
    ...
    builder.defineStreamHandler(async ({ type, id }) => {
        ...
        try {
            const response = await axios.post(`${BRIDGE_URL}/extract-streams`, { url: targetUrl }, { timeout: 120000 });
            ...
            console.error(`Error communicating with bridge for ${targetUrl}:`, error.message);
            return Promise.resolve({ streams: [] });
        }
    });
}
