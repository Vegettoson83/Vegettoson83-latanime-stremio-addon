...
const PROVIDERS = {
    ...
    'voe.sx': async (page) => {
        await page.waitForSelector('script:containing("const t")');
        return page.evaluate(() => {
            const scripts = Array.from(document.querySelectorAll('script'));
            const obfuscatedScript = scripts.find(s => s.textContent.includes('const t='));
            if (!obfuscatedScript) return null;

            try {
                // This is a direct de-obfuscation of the script found in the logs.
                // It extracts the encoded string and simulates the decoding process.
                const fullText = obfuscatedScript.textContent;
                const encodedStringMatch = fullText.match(/const t="([^"]+)"/);
                if (!encodedStringMatch) return null;

                const encoded = encodedStringMatch[1];
                const decoded = encoded.split("").map((char, index) => {
                    const charCode = char.charCodeAt(0) - 32;
                    return charCode >= 0 && charCode < 95 ? String.fromCharCode(32 + (charCode + index) % 95) : char;
                }).join("");

                const urlMatch = decoded.match(/'hls': '([^']+)'/);
                return urlMatch ? urlMatch[1] : null;
            } catch (e) {
                console.error('Voe.sx extractor failed:', e);
                return null;
            }
        });
    }
