...
const { scrapeStreams } = require("./scraper");
...
function defineHandlers(builder) {
    ...
    async function getLatanimeEpisodeUrl(imdbId, season, episode) {
        try {
            ...
            const cleanName = seriesName.split(':')[0].split('(')[0].trim();
            console.log(`Resolved ${imdbId} to series name: "${seriesName}" (clean: "${cleanName}")`);
            ...
            if (searchResults.length > 0) {
                if (searchResults.length === 1) {
                    ...
                    searchResults.each((i, el) => {
                        ...
                        const targetSlug = cleanName.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '');

                        if (slug === targetSlug + '-latino' ||
                            slug === targetSlug + '-castellano' ||
                            slug === targetSlug) {
                            perfectMatch = href;
                            return false;
                        }
    ...
    builder.defineStreamHandler(async ({ type, id }) => {
        ...
        console.log(`[Addon] Scraping streams for target: ${targetUrl}`);

        try {
            const streams = await scrapeStreams(targetUrl);
