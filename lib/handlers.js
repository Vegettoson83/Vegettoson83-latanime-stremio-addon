        ...
        try {
            ...
            if (baseKey) {
                ...
                $('a.play-video').each((i, el) => {
                    ...
                    if (encodedPart) {
                        // The initial URL is the proxy wrapper page
                        providers.push({
                            url: basePlayerUrl + Buffer.from(encodedPart).toString('base64'),
                            title: providerName
                        });
                    }
                });
            ...
            const processProvider = async (provider) => {
                ...
                try {
                    ...
                    // Go to the reproductor proxy page first
                    await page.goto(provider.url, { waitUntil: 'domcontentloaded', timeout: 15000 }).catch(() => {});

                    // Now, from the proxy page, find the real embed URL (usually in an iframe)
                    const finalUrl = await page.evaluate(() => document.querySelector('iframe')?.src).catch(() => null);

                    if (!finalUrl) {
                        console.log(`[Addon] Could not find iframe src for ${provider.title} on proxy page.`);
                        return;
                    }
