...
async function extractGofile(folderUrl) {
    try {
        ...
        const contentR = await axios.get(`https://api.gofile.io/contents/${folderId}?cache=true`, {
            headers: { "Authorization": `Bearer ${token}`, "X-Website-Token": "4fd6sg89d7s6", "User-Agent": "Mozilla/5.0" }
        });
        if (contentR.data.status !== "ok") return null;
        const children = contentR.data.data?.children;
...
app.get('/extract', async (req, res) => {
    ...
    try {
        ...
        const videoUrl = await extractVideoUrl(context, url);
        await context.close();
        if (videoUrl) {
...
app.post('/scrape', async (req, res) => {
    ...
    try {
        ...
        // Special handling for Gofile mirror
        const gofileLink = $('a[href*="gofile.io"]').first().attr('href');
        if (gofileLink) {
            const gofileRes = await extractGofile(gofileLink);
            if (gofileRes) {
                const host = req.get('host');
                const protocol = req.protocol;
                const proxyUrl = `${protocol}://${host}/proxy/file?url=${encodeURIComponent(gofileRes.url)}&token=${encodeURIComponent(gofileRes.token)}`;
                resolvedStreams.push({
                    name: 'Latanime',
                    url: proxyUrl,
                    title: 'Gofile',
